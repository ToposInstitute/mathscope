        ...
        if form.is_valid():
            ...
            for collection in collections:
                documents = Document.objects.filter(
                    ...
                    num_sentences=Count(
                        "sentence", 
                        filter=Q(sentence__lemmas__iregex=regex % query),
                    )
                ).annotate(
                    title_match=Case(
                ...
                total_count += count
                results.append((collection, QueryResults(documents[:10], query),
                                count))

            context['links'] = list(get_wikidata(query)[:5]) + \
                    list(get_nlab(query)[:5])
